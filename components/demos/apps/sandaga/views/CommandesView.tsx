...
const field = "min-h-12 w-full rounded-lg border border-[var(--sandaga-line)] bg-white px-3 py-2.5 text-base outline-none transition placeholder:text-[var(--sandaga-muted)] focus:border-[var(--sandaga-accent)] focus:ring-2 focus:ring-[var(--sandaga-accent-soft)] md:text-sm";
  ...
  return (
    <div className="screen-in space-y-4 @2xl:space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button type="button" onClick={() => setOpen((value) => !value)} className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-[var(--sandaga-deep)] px-4 py-2.5 text-sm font-bold text-white transition hover:bg-[var(--sandaga-accent)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--sandaga-accent)] focus-visible:ring-offset-2">{open ? <X className="size-4" aria-hidden /> : <Plus className="size-4" aria-hidden />}{open ? "Fermer" : "Nouveau dépôt"}</button>
      </div>
      ...
      {open && (
        <form onSubmit={submit} className="screen-in rounded-xl border border-[var(--sandaga-line)] bg-[var(--sandaga-panel)] p-4" aria-label="Enregistrer un dépôt">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3"><label className="flex min-h-11 items-center gap-2 text-xs font-bold"><input type="checkbox" checked={express} onChange={(e) => setExpress(e.target.checked)} className="size-5 accent-[var(--sandaga-signal)]" />Service express</label><p role="alert" className="text-xs text-[var(--sandaga-late)]">{error}</p><button type="submit" className="min-h-11 w-full rounded-lg bg-[var(--sandaga-deep)] px-4 py-2 text-sm font-bold text-white transition hover:bg-[var(--sandaga-accent)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--sandaga-accent)] @sm:ml-auto @sm:w-auto">Enregistrer</button></div>
        </form>
      )}
                ...
                {column.map((order) => {
                  ...
                  return (
                    <li key={order.id} className={`rounded-lg border border-[var(--sandaga-line)] bg-white p-3 ${lateOrder ? "border-l-2 border-l-[var(--sandaga-late)]" : ""}`}>
                      ...
                      <div className="mt-3 flex items-center justify-between gap-2 border-t border-dashed border-[var(--sandaga-line)] pt-2.5">
                        ...
                        <label className="relative inline-flex items-center">
                          ...
                          <select
                            ...
                            className="min-h-11 max-w-36 appearance-none rounded-full border border-[var(--sandaga-line)] bg-white bg-[right_0.5rem_center] bg-no-repeat py-2 pl-5 pr-6 text-base font-bold text-[var(--sandaga-ink)] outline-none transition focus:border-[var(--sandaga-accent)] focus:ring-2 focus:ring-[var(--sandaga-accent-soft)] md:min-h-0 md:max-w-33 md:py-1 md:text-[10px]"
                      ...
                      {stage.id === "prete" && <button type="button" onClick={() => onNotify(order)} className="mt-2.5 flex min-h-11 w-full items-center justify-center gap-1.5 rounded-lg bg-[var(--sandaga-accent-soft)] py-2 text-[11px] font-bold text-[var(--sandaga-accent)] transition hover:brightness-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--sandaga-accent)]"><MessageSquareText className="size-3.5" aria-hidden />Prévenir par SMS</button>}
                      <button type="button" onClick={() => onAdvance(order.id)} className={`mt-2 flex min-h-11 w-full items-center justify-center gap-1.5 rounded-lg py-2 text-[11px] font-extrabold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--sandaga-accent)] ${stage.id === "prete" ? "bg-[var(--sandaga-ready)] text-white hover:brightness-95" : "border border-[var(--sandaga-line)] bg-white text-[var(--sandaga-ink)] hover:border-[var(--sandaga-deep)] hover:bg-[var(--sandaga-deep)] hover:text-white"}`}>
