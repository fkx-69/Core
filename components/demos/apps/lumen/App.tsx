...
export default function LumenApp({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div
      className={`${lumenBody.variable} ${lumenDisplay.variable} @container min-h-[720px] bg-[var(--lumen-canvas)] text-[var(--lumen-ink)] [--lumen-canvas:#f7f6f3] [--lumen-copper-deep:#3f4540] [--lumen-copper-soft:#e9ede8] [--lumen-copper:#667068] [--lumen-gold:#8e8779] [--lumen-grid:#efede8] [--lumen-ink:#292b29] [--lumen-line:#e4e2dd] [--lumen-muted:#777873] [--lumen-olive:#788174] [--lumen-panel:#ffffff] [--lumen-sand:#d9d6cf] [font-family:var(--font-lumen-body)] selection:bg-[var(--lumen-copper-soft)]`}
    >
          ...
          <header className="border-b border-[var(--lumen-line)] px-4 py-3 @2xl:px-6 @4xl:px-8">
            <div className="flex items-center gap-3">
              ...
              <div className="relative min-w-0 flex-1 @3xl:max-w-sm">
                ...
                <input id="lumen-search" type="search" value={query} onChange={(e) => setQuery(e.target.value)} placeholder={view === "sales" ? "Rechercher un client, un produit…" : "Rechercher un produit, une référence…"} className="w-full rounded-lg border border-[var(--lumen-line)] bg-[var(--lumen-panel)] py-2 pl-10 pr-4 text-xs outline-none placeholder:text-[var(--lumen-muted)] focus:border-[var(--lumen-copper)] focus:ring-2 focus:ring-[var(--lumen-copper-soft)]" />
              </div>
              ...
              <button type="button" onClick={reset} className="grid size-9 place-items-center rounded-lg border border-[var(--lumen-line)] text-[var(--lumen-muted)] transition hover:bg-[var(--lumen-canvas)] hover:text-[var(--lumen-ink)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--lumen-copper)]" aria-label="Réinitialiser Lumen" title="Réinitialiser"><RotateCcw className="size-4" aria-hidden /></button>
            ...
            <nav className="mt-3 flex gap-2 @2xl:hidden" aria-label="Vues Lumen">
              {nav.map(({ id, label, icon: Icon, count }) => <button key={id} type="button" onClick={() => changeView(id)} aria-current={view === id ? "page" : undefined} className={`inline-flex items-center gap-1.5 rounded-lg border px-3.5 py-1.5 text-xs font-bold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--lumen-copper)] ${view === id ? "border-[var(--lumen-ink)] bg-[var(--lumen-ink)] text-white" : "border-[var(--lumen-line)] bg-[var(--lumen-panel)] text-[var(--lumen-muted)]"}`}><Icon className="size-3.5" aria-hidden />{label}{count ? <span className={`rounded-full px-1.5 text-[10px] ${view === id ? "bg-white/15 text-white" : "bg-[var(--lumen-copper-soft)] text-[var(--lumen-copper-deep)]"}`}>{count}</span> : null}</button>)}
            </nav>
          </header>
